  ...
  boards(limit: 5) {
    ...
    <script>
        ...
        function findProjectDates(project) {
            ...
            // Try to extract actual dates from items
                ...
                project.items_page.items.forEach(item => {
                    if (item.column_values) {
                        item.column_values.forEach(columnValue => {
                            if (['date', 'timeline'].includes(columnValue.type) && columnValue.text) {
                                // Try to parse dates
                                if (columnValue.type === 'timeline' && columnValue.text.includes(' - ')) {
                                    const [start, end] = columnValue.text.split(' - ');
                                    if (!dateInfo.startDate) dateInfo.startDate = start.trim();
                                    if (!dateInfo.endDate) dateInfo.endDate = end.trim();
                                } else if (columnValue.type === 'date' && columnValue.text) {
                                    if (!dateInfo.startDate) dateInfo.startDate = columnValue.text;
                                }
                            }
                        });
                    }
